...
func (c *secretCache) initKeys() error {
	...
	pkcs8PrivateKey, err := decryptWith(c.data.Sync.Profile.PrivateKey, c.key, c.macKey)
	if err != nil {
		return err
	}
...
func (c *secretCache) decryptStr(orgID *uuid.UUID, s CipherString) (string, error) {
	dec, err := c.decrypt(orgID, s)
	if err != nil {
		return "", err
	}
...
func (c *secretCache) decryptListStr(orgID *uuid.UUID, s ...CipherString) ([]string, error) {
	var ret []string
	for _, v := range s {
		dec, err := c.decryptStr(orgID, v)
		if err != nil {
			return nil, err
		}
		ret = append(ret, dec)
...
func (c *secretCache) decrypt(orgID *uuid.UUID, s CipherString) ([]byte, error) {
	if s.IsZero() {
		return nil, nil
	}
